 ...
 * 每次失败打日志，故障发生/恢复时向所有用户推送一条告警（一次故障只推一次，防刷屏）。
 * 探测结果同时暴露在 /health 端点（getHealthProbeStatus）。
 *
...
export function startHealthProbe(store: Store, data: DataProvider, channels: Channel[]): void {
  const { code, intervalMinutes } = config.healthProbe;
  ...
  const tick = async (): Promise<void> => {
    ...
    if (!probeError) {
      if (!status.ok) {
        ...
        await notifyAll('✅【数据源恢复】行情接口已恢复正常。');
      }
      status.ok = true;
