 ...
 * 一次性并发拉取 行情/估值、公司资料、资金流、技术指标、财报、新闻 六块结构化数据，
 * 格式化为紧凑文本回传给 LLM，由 LLM 分维度客观解读（解读与免责声明由 SYSTEM_PROMPT 约束）。
...
const skill: Skill = {
  name: 'analyze_stock',
  description:
    '个股多维分析（F5-2）：一次性聚合指定股票的行情/估值、公司资料、资金流、技术指标、近两期财报与最新新闻，' +
  ...
  async execute(args, ctx) {
    ...
    // 六块并发、独立降级：单块失败只影响该块（与详情聚合 /api/stocks/:code 同一模式）
    const [quote, profile, fundFlow, indicators, financials, news] = await Promise.allSettled([
      data.getQuote(code),
