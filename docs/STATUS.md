...
- 方案一（首选，成本最低）：主服务监听改为 `0.0.0.0`，手机与 PC 同局域网直连
  （`http://<PC局域网IP>:18790`）；ACCESS_TOKEN 口令鉴权已有，风险可控，但口令会走
  明文 HTTP——仅限家庭可信网络，文档须写明。
- 方案二（公网/随时可用）：服务端公网部署 + HTTPS。**前置硬依赖 S3-3 多用户体系**
  （审计 A-601：同口令无身份隔离）与 A-508（微服务 token），不要跳过。

**技术路线候选**（留给实现 agent 选型实测，建议从路线 A 起步验证）：

| 路线 | 方案 | 成本 | 说明 |
|---|---|---|---|
| A（推荐起步） | **WebView / TWA 壳**：Android Studio 原生 WebView 或 Bubblewrap(TWA) 包装现有网页 | 最低 | 现有页面零改动全可用；口令输入走现有 localStorage 机制；服务端地址做成首屏可配置 |
...
**建议拆分**：F7-1 后端局域网可达（监听地址可配置 + 文档）→ F7-2 WebView/TWA 壳
跑通（地址/口令配置页 + 全页面验收）→ F7-3 体验加固（本地通知、后台轮询、图标/启动屏）。
落点建议：新目录 `android/`（或独立仓库），不动现有 `src/` 与 `public/`；
如主服务加监听地址配置，走正常分支 + PR 流程。
