...
| S3-4 | 进程管理加固（P9 善后）：一键启动脚本（双服务同起）+ /health 暴露 data-service 连通性与进程启动时间/git sha；（可选）data-service 健康探针 | 未做。2026-09-16 P9 事故后立项，重启服务即可临时恢复 |
...
### P9 进程管理失控：主服务热重载失效跑旧代码 + data-service 停机无感知（2026-09-16 用户发现大面积"没有数据"，排查实锤）
- **影响**：用户 2026-09-16 上午看到——基金页、快讯页、自选股详情页的公司资料/分时走势/
  技术指标/新闻/公告/财报/资金流/分红全部无数据，行情卡涨跌停价/52 周高低恒显示 —。
  涉及面 = 除实时行情外的几乎全部数据功能。
- **根因（两个独立问题叠加）**：
  1. **主服务在跑旧代码**：进程 PID 26168 是 2026-09-15 20:48 启动的（F3-4 合并前），
     tsx watch 此后**从未热重载**——当晚 F3-4/F3-5/F3-6 三次合并、次日 F5-1 的全部文件
     变更都没生效。静态页面从磁盘实时读取是最新前端，API 却是旧代码，**前后端版本错位**
     症状极有迷惑性：页面有"涨停价/52 周"行但恒 —（旧 pickQuote 不解析这些字段）、
     资金流/分红卡整块消失（旧聚合无这两块）、/intraday 与 /indicators 路由 404。
     实证：`/api/stocks/002594` 响应 quote 无 limitUp 等 F3-6 键、fundFlowError 键不存在；
     同时段腾讯源实测 47/48=92.73/75.87 有值——字段缺失不是数据源问题。
  2. **data-service 完全未运行**：8000 端口无监听、无任何 python 进程，停机时间与原因
     不可考（无守护、无日志）。详情页公司资料/新闻/公告/财报、/funds 基金页、/news 快讯页
     全部依赖它，按设计逐块降级为错误提示——但主服务 /health **不反映 data-service 状态**，
     无告警，用户只能在页面上逐块发现。
- **建议修法**：
  1. **立即（下一个 agent 第一件事）**：重启两个服务——先 `netstat -ano | findstr :18790`
     /`:8000` 杀净旧进程再启动（PITFALLS 旧进程条目有完整操作），重启后打特征端点验证
     （`/api/stocks/002594` 的 quote 应含 limitUp/limitDown/week52 键，/api/funds/rank 应有数据）。
  2. **短期**：加一键启动脚本（如 `scripts/dev-up.bat` 或 npm 并发脚本同时拉起主服务 +
     data-service），README/CLAUDE.md 同步；主服务 `/health` 增加 data-service 连通性探测
     与**本进程启动时间/git sha**，让"进程是不是最新"一眼可辨（本次若 /health 暴露版本号
     就不会误诊为数据源问题）。
  3. **中期（可选）**：健康探针扩展到 data-service（复用 healthProbe 模式探测其 /health
     或 /trade-calendar），连续失败走 notify 推送告警。

### P10 盘前时段（9:30 前）行情卡部分字段显示 — / 0.00——设计内行为，不是故障（2026-09-16 排查确认）
- **影响**：盘前打开详情页，今开/最高/最低/成交额显示 —，换手率/量比显示 0.00%/0.00，
  用户容易误判为接口故障（本次即与 P9 的真故障混在一起）。
- **根因**（2026-09-16 09:18 对腾讯降级源实测 sz002594）：盘前腾讯 open/high/low 返回
  `"0.00"`（字符串，非空串），`opt()` 按 `v > 0` 判置 undefined——这是 A-310"不显示假 0"
  的设计；成交额 0 → fmtCap 显示 —；换手率/量比 `"0.00"` 属合法 0 保留显示。
  **涨停/跌停价腾讯盘前实测有值**（92.73/75.87），若页面显示 — 先怀疑进程代码版本（P9），
  不要怀疑数据源。52 周高低腾讯永远缺失（F3-6 已知设计）。
- **建议修法**：无需修复；可选优化——盘前量比/换手率的 0 值显示为 —（需引入"盘前"
  时段判断，优先级低）。排查纪律已写入 PITFALLS 腾讯条目。
