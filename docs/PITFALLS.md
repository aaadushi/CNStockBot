...
- **现象**：接入板块轮动（F6-3）时发现——① `ak.stock_board_industry_name_em()` 的输出
  ...
  （只有名称），没法和成分股/走势端点联动；② 验证期间 push2his（板块日 K）对本机
  从"可用"突然变成"连接被掐"（RemoteDisconnected），过一阵又可能恢复——限流是
  **间歇性**的，一次实测成功不代表端点稳定。
- **根因**：AKShare 封装按自己页面的需要裁剪列，本项目要的联动字段（成交额/领涨股代码/
  板块代码）恰好在被丢弃之列；东财 IP 限流按请求频率动态收放。
- **解法**：排行/资金流/成分股按 AKShare 同参数同字段**直连 clist**（`_clist_paginated`，
  push2 → push2delay 宿主降级，翻页间隔 0.3s）；板块日 K 的 AKShare 封装支持直传 BK 代码
  （`re.match(r"^BK\d+")` 分支），传代码可跳过其内部"名称→代码"的额外一次排行表请求。
  push2his 无已知延时镜像，限流期板块日 K 只能返回结构化 502（页面走势图区块独立降级）。
- **涉及文件**：`data-service/main.py`（板块轮动监控节）
- **预防**：接 AKShare 榜单类封装前先 diff 它"抓取的字段"与"最终输出的列"——丢弃在
  `temp_df = temp_df[[...]]` 那一步；东财接口实测要隔几分钟重试一次再下"可用/不可用"结论。
...
### [2026-09-22] baostock 免费服务端会"登录用户过多"拒绝登录；批量日 K 改用腾讯 fqkline 主源
- **现象**：首日回填跑约 4 小时后（22:00 起）baostock 先是大面积"网络接收错误"，
  次日早晨连 `bs.login()` 都被秒拒，错误码 10001011"登录用户过多，请稍后再试"——
  持续 12+ 小时，回填卡死在 22%。
- **根因**：baostock 免费服务端有会话总数上限，高峰期/夜间数据处理期挤不进去；
  不是封号（错误码与限流文案明确），但恢复时间不可控。
- **解法**：批量日 K 改双源——**腾讯 `web.ifzq.gtimg.cn/appstock/app/fqkline/get`
  （`param=sh600519,day,,,640,qfq`）升为主源**（免登录、单次 ~640 根、实测 1.1s/票、
  无限流前科），baostock 降级为懒登录兜底（登录失败一次本轮禁用）。腾讯口径差异：
  volume 已是"手"（**不要**再 ÷100）；无 amount/turnover 列（置 None）；
  无 change_pct（收盘价环比补算）；无效代码返回 code=0 但无 qfqday 键（判空）。
- **涉及文件**：`data-service/main.py`（_tx_fetch_bars / _DualFetcher）
- **预防**：免费批量数据源都要假定"会不可用"，更新器按双源 + 断点续跑设计；
  接入新源时先用已知数值票（600519）核对单位与字段顺序。
