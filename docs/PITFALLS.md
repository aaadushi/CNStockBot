...
### [2026-09-15] push2 限流期可用 push2delay 同构接口验证字段；腾讯估值下标交叉核对结果
- **现象**：push2 对本机 IP 断连限流持续超过 20 小时未解除（前一日录 fixture 触发），
  期间无法实测东财新字段编码。
- **根因**：东财 IP 级限流恢复时长不确定（几小时到 1 天+）；push2 与 push2his 限流独立。
- **解法**：`push2delay.eastmoney.com` 提供**同构的延时行情接口**（同 URL 路径/字段编码，
  仅行情延时约 15 分钟），限流期间可用来验证字段编码与录制 fixture——
  注意它是延时行情，**不要把它切换为数据源**。本次实测核对结果（600519，与腾讯接口交叉一致）：
...
- **涉及文件**：`src/data/eastmoney.ts`（FIELDS 与解析）、`src/data/tencent.ts`
- **预防**：接东财新字段时若 push2 被限流，用 push2delay 验证；市值类大数值字段先确认是否缩放。
