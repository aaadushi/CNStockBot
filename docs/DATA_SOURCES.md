...
**策略回测本地计算（/backtest/{code}，F5-6，2026-09-22；非外部接口）**

输入 = 本地日 K 库单票最近 N 根 bar（days 参数默认 750、范围 120~750）；
信号掩码与扫描**同一公式集**（`_wide_indicators`，pandas Series 与 DataFrame 语义一致），
策略条件与 7 个预设扫描策略逐条相同。交易规则（响应 rules 字段透明披露）：

- **信号日**：状态类条件（RSI 超卖、MA 多头排列等）取上升沿（条件首次成立日）防同一
  状态每日重复开仓；事件类条件（MACD/均线金叉）掩码本身即沿
- **买入**：信号日次日开盘价 ×(1+0.1% 滑点)；固定本金 10 万元按整手（100 股）买入，
  资金不足一手（高价股）跳过该信号
- **T+1**：买入当日不可卖出，最早次日可卖
- **卖出**（先到先触发）：止损 = 买入价 ×(1-p%)，盘中最低价触及即成交，
  成交价 = min(当日开盘价, 止损价)（跳空低开按开盘价）；持有期满 = 买入后第 hold_days
  个交易日（白名单 5/10/20/60）收盘价卖出；数据末端强制平仓（reason=data_end）
- **费用**：佣金万 2.5 双边（单笔最低 5 元）+ 卖出印花税 0.05%（2023-08-28 起税率）
  + 双边滑点 0.1%
- **单持仓**：同一时间只持有一笔，持仓期间新信号忽略（skippedSignals 计数）
- **统计口径**：胜率/平均收益/盈亏比等只统计闭环交易（data_end 强平不计入，与 F6-1
  "窗口不完整不计入"同原则）；净值曲线按日盯市（现金+持仓市值），含全部交易；
  基准 = 同区间买入持有（首根收盘买入）；最大回撤基于净值曲线
- 结果缓存键含数据 asOf（同扫描）；北交所代码 400（本地库仅沪深）；
  库中无票 404；bar 数 < 90 回 422
- 红线：历史业绩不代表未来，响应带 disclaimer，展示层必须保留
